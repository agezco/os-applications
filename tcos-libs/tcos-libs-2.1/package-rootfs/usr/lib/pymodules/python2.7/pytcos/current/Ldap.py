...
from ObjectManager import ObjectManager as om
...
class Ldap():
    ...
    class SecondaryLdapConnection:
        def __init__(self, ldap_url=None):
            if ldap_url:
                ...
                self.LDAP_URL = self.u.getLdapUrl()

            client_dn = Ldap().getClientDn(self.u.getMac(), self.LDAP_URL)
    ...
    def getGroupOfUniqueNamesDn(self,
        ...
        try:
            ...
            return self.u.unifyList(group_of_unique_names_dn)
        except:
    ...
    def getUsergroupsDn(self, user_dn, ldap_url):
        ...
        if (server_type == self.DirectoryType.RFC or \
            ...
            usergroups = self.getMemberUidDn(self.u.getUsername(), ldap_url)
        else:
            usergroups = []
